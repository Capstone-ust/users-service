package com.sr.users_service.controller;
...
import com.sr.users_service.config.CustomUserDetails;
import com.sr.users_service.entity.Users;
import com.sr.users_service.repository.UsersRepository;
import com.sr.users_service.service.AuthService;
import com.sr.users_service.service.JwtService;
...
public class UsersController {
    ...
    public ResponseEntity<String> registerPerson(@RequestBody Users user) {
        try {
    ...
    @PostMapping("/login")
    public ResponseEntity<Map<String, String>> getToken(@RequestBody Users authRequest) {
        Users user = authService.findByUsername(authRequest.getUsername());
        if (user.getStatus() == 0) {
    ...
    public ResponseEntity<List<Users>> getUsersWaitingForApproval() {
        List<Users> usersWaitingForApproval = authService.findUsersWaitingForApproval();
